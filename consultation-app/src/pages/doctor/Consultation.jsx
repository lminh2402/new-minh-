...
import { Search, Video, PhoneOff, Paperclip, Send, Image as ImageIcon, Brain, FileText, History, Clock, CheckCircle2, AlertTriangle } from 'lucide-react';
...
const Consultation = () => {
  ...
  return (
    <>
      ...
      {/* Cột 2: Khung Chat Bác sĩ - Bệnh nhân */}
      <section className="flex-1 min-w-[400px] bg-[#f8fafc] flex flex-col border-r border-gray-200 relative">
        <div className="h-16 px-6 bg-white border-b border-gray-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-base">A</div>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-xs font-bold transition-colors">
              <PhoneOff size={16} /> Kết thúc khám
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex gap-3 max-w-[85%] self-end flex-row-reverse">
              <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center shrink-0 text-white font-bold text-xs shadow-sm">BS</div>
            ...
            <div className="flex gap-3 max-w-[85%]">
              <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center shrink-0 text-indigo-700 font-bold text-xs">A</div>
              <div className="bg-white border border-gray-200 p-3.5 rounded-2xl rounded-tl-sm shadow-sm">
        ...
        <div className="p-4 bg-white border-t border-gray-200 shrink-0">
          <div className="flex items-end gap-2 p-1.5 bg-gray-50 border border-gray-200 rounded-xl focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
            <div className="flex gap-1 pb-1 px-1">
              <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"><Paperclip size={18} /></button>
              <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"><ImageIcon size={18} /></button>
            </div>
            <textarea placeholder="Nhập tin nhắn tư vấn cho bệnh nhân..." className="flex-1 max-h-24 min-h-[38px] bg-transparent border-none focus:ring-0 resize-none py-2 text-sm outline-none text-gray-800" rows="1"></textarea>
            <button className="p-2 mb-0.5 mr-0.5 bg-blue-600 text-white hover:bg-blue-700 rounded-lg shadow-sm transition-transform active:scale-95"><Send size={16} /></button>
          </div>
          <div className="mt-3 relative">
            <textarea placeholder="Ghi chú nhanh / Chẩn đoán sơ bộ (Chỉ bác sĩ xem được)..." className="w-full h-16 p-3 bg-[#fefce8] border border-yellow-200 rounded-xl text-xs text-gray-800 focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 outline-none resize-none placeholder-yellow-700/40 font-medium"></textarea>
          </div>
        </div>
      </section>

      {/* Cột 3: Trợ lý AI & Bệnh án */}
      <section className="w-[25%] min-w-[280px] bg-white flex flex-col z-0 shadow-[-4px_0_15px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5 scrollbar-hide bg-[#f8fafc]/50">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <h3 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2"><FileText size={16} className="text-blue-500" /> AI Tóm tắt tình trạng</h3>
            <ul className="space-y-2.5 text-xs text-gray-700">
              <li className="flex gap-2"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 shrink-0"></span><span className="leading-relaxed"><strong className="text-gray-900 font-semibold">Triệu chứng chính:</strong> Đau bụng hố chậu phải dữ dội, buồn nôn, sốt 38.5°C.</span></li>
              <li className="flex gap-2"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 shrink-0"></span><span className="leading-relaxed"><strong className="text-gray-900 font-semibold">Thời gian phát bệnh:</strong> Rạng sáng nay (~5 tiếng trước).</span></li>
              <li className="flex gap-2"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 shrink-0"></span><span className="leading-relaxed"><strong className="text-gray-900 font-semibold">Mức độ đau:</strong> 8/10 (Nghiêm trọng).</span></li>
            </ul>
          </div>
          <div className="bg-emerald-50/80 rounded-xl p-4 shadow-sm border border-emerald-100/80">
            <h3 className="text-sm font-bold text-emerald-800 mb-3 flex items-center gap-2"><CheckCircle2 size={16} className="text-emerald-600" /> Gợi ý xử trí tại nhà từ AI</h3>
            <ul className="space-y-2.5 text-xs text-emerald-900/90">
              <li className="flex gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1 shrink-0 shadow-sm"></span><span className="leading-relaxed font-medium">Nghi ngờ Viêm ruột thừa cấp (Appendicitis). Đã xếp loại CẤP CỨU.</span></li>
              <li className="flex gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1 shrink-0 shadow-sm"></span><span className="leading-relaxed">Đã khuyên bệnh nhân nhịn ăn uống, KHÔNG dùng thuốc giảm đau tự ý.</span></li>
            </ul>
          </div>
          <button className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-white hover:bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 transition-all shadow-sm active:scale-95">
            <History size={16} className="text-gray-500" /> Lịch sử chat gốc với AI
          </button>
        </div>
        <div className="p-4 mt-auto bg-white border-t border-gray-100 shrink-0">
          <button className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-white hover:bg-red-50 border border-red-200 hover:border-red-400 text-red-600 rounded-xl text-xs font-bold transition-all shadow-sm active:scale-95 group">
            <AlertTriangle size={16} className="group-hover:scale-110 transition-transform" /> Báo cáo lỗi chẩn đoán AI
          </button>
        </div>
      </section>
    </>
